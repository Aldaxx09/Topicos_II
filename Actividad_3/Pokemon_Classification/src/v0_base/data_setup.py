...
def create_dataloaders(
  ...
  """
  # 1. Crear Datasets usando ImageFolder
  # ImageFolder escanea la carpeta y usa los nombres de las subcarpetas como etiquetas (clases).
  train_data = datasets.ImageFolder(train_dir, transform=transform)
  test_data = datasets.ImageFolder(test_dir, transform=transform)

  # 2. Obtener nombres de clases
  # Extrae la lista de clases (ej: ['Bulbasaur', 'Charmander']) basada en las carpetas.
  class_names = train_data.classes

  # 3. Convertir Datasets en DataLoaders
  # El DataLoader es el "camarero" que sirve los datos en lotes al modelo.
  train_dataloader = DataLoader(
      train_data,
      batch_size=batch_size,
      shuffle=True, # IMPORTANTE: Barajar entrenamiento para que el modelo no memorice el orden.
